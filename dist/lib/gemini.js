...
async function waitForCallSlot() {
    ...
    if (_lastCallAt > 0 && elapsed < MIN_CALL_INTERVAL_MS) {
        ...
        logger_1.logger.debug({ waitMs: wait }, 'Gemini call guard: throttling request');
...
function markQuotaExhausted() {
    ...
    logger_1.logger.error({ cooldownUntil: new Date(_quotaExhaustedUntil).toISOString() }, 'Gemini daily quota exhausted — circuit breaker open. Calls will be skipped for 1 hour.');
}
// Returns true when the error is a daily quota exhaustion (not just RPM)
function isDailyQuotaError(errMsg) {
    return (errMsg.includes('GenerateRequestsPerDayPerProject') ||
...
async function callGemini(systemPrompt, userPrompt, maxTokens = 2048) {
    if (isQuotaExhausted()) {
        throw new Error(`Gemini quota circuit breaker is open until ${new Date(_quotaExhaustedUntil).toISOString()}. ` +
...
async function callGeminiJSON(systemPrompt, userPrompt, maxTokens = 2048) {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
        try {
            ...
            logger_1.logger.warn({ attempt, retryAfterMs: delayMs }, `Gemini 429 (RPM) — waiting ${(delayMs / 1000).toFixed(0)}s before retry`);
            await new Promise(resolve => setTimeout(resolve, delayMs));
        }
    }
    logger_1.logger.error({ err: lastError?.message?.slice(0, 300) }, 'Gemini JSON call failed permanently');
    throw lastError;
}
